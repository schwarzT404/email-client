...
class MessageManager:
    ...
    def init_message_tables(self):
        ...
        try:
            ...
            print("✅ Tables de messages créées")
        ...
        try:
            ...
            scenarios = [
                ...
                ("Retard Livraison Standard", "retard_livraison", 3, 
                 ...
                 "Bonjour, ma commande était prévue pour arriver mais je n'ai toujours rien reçu. Pouvez-vous me dire où elle en est ? Merci.", 
                 "tracking_info", 0.3),
                
                ("Retard Livraison Urgent", "retard_livraison", 4,
                 "URGENT - Colis en retard pour événement important",
                 "C'est urgent ! Ma commande devait arriver avant un événement important. Le retard va compromettre mon projet. Que pouvez-vous faire ?",
                 "priority_handling", 0.15),
                
                # Produits défectueux
                ("Produit Cassé Standard", "produit_defectueux", 3,
                 "Produit reçu endommagé",
                 "Le produit que j'ai reçu est arrivé cassé. L'emballage semblait intact mais l'article à l'intérieur est inutilisable. Comment procéder pour un échange ?",
                 "replacement", 0.2),
                
                ("Produit Non Conforme", "produit_defectueux", 4,
                 "Produit ne correspond pas à la description",
                 "Le produit reçu ne correspond absolument pas à ce qui était décrit sur votre site ! C'est de la fausse publicité. Je veux un remboursement immédiat.",
                 "refund", 0.1),
                
                # Demandes de remboursement
                ("Remboursement Simple", "remboursement", 2,
                 "Demande de remboursement",
                 "Bonjour, j'aimerais être remboursé(e) de ma commande. Le produit ne me convient pas. Quelle est la procédure à suivre ?",
                 "refund_process", 0.15),
                
                ("Remboursement Urgent", "remboursement", 4,
                 "Remboursement URGENT - Problème financier",
                 "J'ai besoin d'être remboursé(e) de ma commande de toute urgence suite à un problème financier imprévu. C'est vraiment urgent !",
                 "priority_refund", 0.05),
                
                # Informations commande
                ("Info Commande Standard", "information_commande", 2,
                 "Demande d'information sur ma commande",
                 "Pouvez-vous me donner des informations sur ma commande ? J'aimerais connaître le statut et la date de livraison prévue.",
                 "order_status", 0.25),
                
                ("Facture Demandée", "information_commande", 1,
                 "Demande de facture",
                 "Pouvez-vous m'envoyer la facture de ma commande ? J'en ai besoin pour ma comptabilité. Merci d'avance.",
                 "invoice", 0.1),
                
                # Réclamations
                ("Réclamation Service", "reclamation", 4,
                 "Très mécontent du service client",
                 "Je suis extrêmement déçu(e) de votre service client. C'est plusieurs fois que j'ai un problème et personne ne semble s'en préoccuper !",
                 "complaint_handling", 0.08),
                
                ("Réclamation Qualité", "reclamation", 3,
                 "Qualité des produits en baisse",
                 "J'ai remarqué une baisse de qualité dans vos produits récemment. Mes dernières commandes étaient décevantes. Que comptez-vous faire ?",
