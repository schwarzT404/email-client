...
class AdvancedSupportMonitoring:
    ...
    def get_dashboard_data(self):
        ...
        try:
            ...
            # Logs de transactions récents (NOUVEAU)
            try:
                cursor.execute('''
                    SELECT email, action, status, timestamp, details
                    FROM transaction_logs 
                    ORDER BY timestamp DESC 
                    LIMIT 30
                ''')
                transaction_logs = [dict(zip([col[0] for col in cursor.description], row)) for row in cursor.fetchall()]
            except:
                transaction_logs = []
            
            # Emails inconnus - tentatives d'accès refusé (NOUVEAU)
            try:
                cursor.execute('''
                    SELECT email, attempt_count, last_attempt, status, action_type
                    FROM unknown_emails 
                    ORDER BY last_attempt DESC 
                    LIMIT 15
                ''')
                unknown_emails = [dict(zip([col[0] for col in cursor.description], row)) for row in cursor.fetchall()]
            except:
                unknown_emails = []
            
            # Statistiques des tentatives d'accès (NOUVEAU)
            try:
                cursor.execute('''
                    SELECT 
                        COUNT(CASE WHEN action = 'ACCES_REFUSE' THEN 1 END) as acces_refuses,
                        COUNT(CASE WHEN action = 'ACCES_AUTORISE' THEN 1 END) as acces_autorises,
                        COUNT(CASE WHEN action = 'MESSAGE_TRAITE' THEN 1 END) as messages_traites
                    FROM transaction_logs 
                    WHERE DATE(timestamp) = DATE('now')
                ''')
                security_stats = dict(zip([col[0] for col in cursor.description], cursor.fetchone() or [0, 0, 0]))
            except:
                security_stats = {'acces_refuses': 0, 'acces_autorises': 0, 'messages_traites': 0}
...
@app.route('/api/transaction-logs')
def get_transaction_logs():
    """API pour récupérer les logs de transactions"""
    try:
...
@app.route('/api/unknown-emails')
def get_unknown_emails():
    """API pour récupérer les emails inconnus (tentatives d'accès refusé)"""
    try:
        conn = sqlite3.connect('data/crm_ecommerce.db')
        cursor = conn.cursor()
        
        cursor.execute('''
            SELECT email, attempt_count, first_attempt, last_attempt, status, action_type
            FROM unknown_emails 
            ORDER BY last_attempt DESC
        ''')
        ...
        return jsonify({'success': True, 'unknown_emails': emails})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
...
def get_security_stats():
    ...
    try:
        ...
        ''')
        today_stats = [dict(zip([col[0] for col in cursor.description], row)) for row in cursor.fetchall()]
        ...
        cursor.execute('''
            ...
            GROUP BY action
        ''')
        week_stats = [dict(zip([col[0] for col in cursor.description], row)) for row in cursor.fetchall()]
...
if __name__ == '__main__':
    ...
    print("📊 Surveillance Complète des Transactions")
    print("🔒 Monitoring Sécurit&eacute; et Accès")
    print("=====================================")
